    ...
    <form method="POST" action="{{ route('password.update') }}">
        ...
        <div class="p-8 border-t-8 bg-white mb-6 rounded-lg shadow-lg border-green-200">
            <div class="mb-4">
                ...
                @endif
                <input id="email" type="email" name="email" value="{{ $email ?? old('email') }}" required autofocus class="block appearance-none w-full bg-white border border-grey-light hover:border-grey px-2 py-2 rounded shadow">
            </div>
            <div>
            ...
            <div>
                <label for="password-confirm" class="text-gray-700 block mb-2">{{ __('Confirm Password') }}</label>
                @if ($errors->has('password-confirm'))
                    <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-2" role="alert">
                        <p>{{ $errors->first('password') }}</p>
                    </div>
                @endif
                <input id="password-confirm" type="password" name="password_confirmation" required class="block appearance-none w-full bg-white border border-grey-light hover:border-grey px-2 py-2 rounded shadow">
            </div>
            <div class="flex mt-6">
